...
void Marco_UI::Crear(DWL::DhWnd* nPadre, const int cX, const int cY, const int cAncho, const int cAlto, const int cID) {
	...
	BarraTiempoAnimaciones.EventoFormatearValor = [=](std::wstring& Str) {	Str += L" ms"; };
	...
	BarraTiempoToolTips.EventoFormatearValor = [=](std::wstring& Str) {	Str += L" ms"; };
	...
	BarraVideoTiempo.MostrarValor(DBarraEx_MostrarValor_ValorInt);
	BarraVideoTiempo.EventoFormatearValor = [=](std::wstring& Str) {	Str += L" ms"; };

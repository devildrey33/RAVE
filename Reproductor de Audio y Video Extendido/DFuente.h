...
namespace DWL {
	...
	class DFuente_Datos {
      public :
							...
							DFuente_Datos(const int nTam, std::wstring &nNombre, const BOOL nNegrita, const BOOL nCursiva, const BOOL nSubrayado, const BOOL nTachado, HFONT nFuente, const int nAlto) : Tam(nTam), Nombre(nNombre), Negrita(nNegrita), Cursiva(nCursiva), Subrayado(nSubrayado), Tachado(nTachado), Fuente(nFuente), Alto(nAlto) { }
		int					Tam;
		std::wstring		Nombre;
		BOOL				Negrita;
		BOOL				Cursiva;

...
namespace DWL {
	...
	class DFuente {
	   public:	
		...
		inline const int						Tam(void)			{ return _Fuente->Tam;				};
		inline const wchar_t	               *Nombre(void)		{ return _Fuente->Nombre.c_str();	};
		inline const BOOL						Negrita(void)		{ return _Fuente->Negrita;			};
		inline const BOOL						Cursiva(void)		{ return _Fuente->Cursiva;			};
		inline const BOOL						Subrayado(void)		{ return _Fuente->Subrayado;		};
		inline const BOOL						Tachado(void)		{ return _Fuente->Tachado;			};
		...
		inline const int						Alto(void)			{ return _Fuente->Alto; }
												// Fuente para utilizar al pintar las ventanas / controles
		inline HFONT							operator() (void)	{ return _Fuente->Fuente; }
		inline HFONT							Fuente(void)        { return _Fuente->Fuente; }

												// Obtiene el tamaño del texto especificado, utilizando la fuente de esta clase
		inline const SIZE						ObtenerTamTexto(const TCHAR *nTexto)			{ return _Fuente->ObtenerTamTexto(nTexto); };
												// Obtiene el tamaño del string especificado, utilizando la fuente de esta clase
		inline const SIZE						ObtenerTamTexto(std::wstring &nTexto)			{ return _Fuente->ObtenerTamTexto(nTexto); };
												// Obtiene el tamaño del texto especificado. El DC especificado debe tener seleccionada esta fuente.
		inline const SIZE						ObtenerTamTexto(HDC DC, const TCHAR *nTexto)	{ return _Fuente->ObtenerTamTexto(DC, nTexto); };
												// Obtiene el tamaño del string especificado. El DC especificado debe tener seleccionada esta fuente.
		inline const SIZE						ObtenerTamTexto(HDC DC, std::wstring &nTexto)	{ return _Fuente->ObtenerTamTexto(DC, nTexto); };
